...
public class ShootBall extends SequentialCommandGroup {
  ...
  public ShootBall(double angle, double speed, ShooterWheelsSubsystem shooterWheelsSubsystem, HoodSubsystem hoodSubsystem, FeederSubsystem feederSubsystem) {
    addCommands(
      new ParallelCommandGroup(
        new InstantCommand(() -> shooterWheelsSubsystem.set(speed)),
        new InstantCommand(() -> hoodSubsystem.setSetpoint(angle))
      ),
      new ParallelDeadlineGroup(
          new WaitUntilCommand(() -> (shooterWheelsSubsystem.atSpeed() && hoodSubsystem.atAngle())),
          new ParallelDeadlineGroup(
            new WaitCommand(3),
            new FeederForward(feederSubsystem)
          ),
          new HoodSetAngle(hoodSubsystem, angle),
          new ShooterWheelsSetSpeed(shooterWheelsSubsystem, speed)
        ),
      new ParallelCommandGroup(
